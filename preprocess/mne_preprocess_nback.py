...
def watch(data):
    ...
    for idx,i in enumerate(data):
        ...
        plt.plot(i)
        plt.show()
    plt.plot(numpy.stack(list(range(len(data.flatten()))))/len(data.flatten())*14,data.flatten())
    plt.show()
def reform(data):
...
def mnePreprocess(raw_path):
    ...
    #时间截取
    #raw.crop(tmin=t_start,tmax=t_stop,verbose='WARNING')
    ...
    raw_0.crop(tmin=960,tmax=1020,verbose='WARNING')
    raw.crop(tmin=480,tmax=540,verbose='WARNING')
    ...
    #eog_inds,eog_scores=ica.find_bads_eog(raw,['AF3','AF4'],measure='correlation',threshold=0.9,verbose='WARNING')
    ...
    save_dict['data']=raw_0.get_data()
    full_write_path=write_path+raw_path[:-4]+'-0.mat'
    scipy.io.savemat(full_write_path,save_dict)

    save_dict={}
    save_dict['data']=raw.get_data()
    full_write_path=write_path+raw_path[:-4]+'-1.mat'
    scipy.io.savemat(full_write_path,save_dict)
